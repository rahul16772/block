...
@lru_cache
def get_name_from_str(s: str, no_spaces=False):
    # ~200 entries for both lists; so 2 hex digits.
    ints = hex_to_ints(hashlib.md5(s.encode()).hexdigest(), 2)
...
def search_for_name(strs: Sequence[str], name):
    for s in strs:
        if name == get_name_from_str(s):
            return s
    return None